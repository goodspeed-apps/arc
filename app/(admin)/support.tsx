        ...
        <View style={{ flex: 1, backgroundColor: colors.background }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 16 }}>
            ...
            <TouchableOpacity onPress={() => setActive(null)}>
              <Text style={{ color: colors.textOnPrimary, backgroundColor: colors.primary, padding: 6, borderRadius: 6 }}>Close</Text>
            </TouchableOpacity>
          </View>
          {msgLoading ? (
            <ActivityIndicator />
            ...
            <ScrollView style={{ flex: 1, padding: 16 }}>
              {messages.map(m => (
                <View key={m.id} style={{ marginBottom: 12, alignItems: m.author_role === 'admin' ? 'flex-end' : 'flex-start' }}>
                  <View style={{ backgroundColor: m.author_role === 'admin' ? colors.primary : colors.surface, borderRadius: 10, padding: 10, maxWidth: '80%' }}>
          ...
          <View style={{ padding: 16, flexDirection: 'row', gap: 8 }}>
            <TextInput
              ...
              style={{ flex: 1, borderWidth: 1, borderColor: colors.border, borderRadius: 8, padding: 10, color: colors.text }}
              multiline
            />
