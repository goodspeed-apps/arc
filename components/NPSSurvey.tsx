...
/**
 * NPSSurvey, Net Promoter Score modal.
 *
 * Usage:
 *   const { shouldShow, dismiss } = useNPSSurvey(7);
...
export function NPSSurvey({ visible = false, onClose, userId, isPaidUser }: NPSSurveyProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      captureEvent('nps_survey_submitted', { score, category });
  ...
  }, [score, submitting, userId, followUp, isPaidUser, onClose]);

  const handleDismiss = () => {
    captureEvent('nps_survey_dismissed', { score });
    onClose?.();
  };

  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={handleDismiss}>
      <Pressable style={{ flex: 1, backgroundColor: colors.overlay, justifyContent: 'flex-end' }} onPress={handleDismiss}>
        <Pressable
          style={{ backgroundColor: surfaceDark, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 }}
          onPress={() => {}}
        >
          {/* Header */}
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <Text style={{ color: '#FFFFFF', fontSize: 18, fontWeight: '700' }}>Quick Question</Text>
          ...
          {submitted ? (
            <View style={{ alignItems: 'center', paddingVertical: 32 }}>
              ...
              <Text style={{ color: '#FFFFFF', fontSize: 18, fontWeight: '700', marginTop: 16 }}>Thanks!</Text>
              <Text style={{ color: colors.textSecondary, marginTop: 8 }}>Your feedback helps us improve.</Text>
            </View>
          ) : (
            <>
              <Text style={{ color: '#FFFFFF', fontSize: 15, marginBottom: 20 }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                {Array.from({ length: 11 }, (_, i) => (
                  ...
                  >
                    <Text style={{ color: score === i ? '#FFFFFF' : colors.textSecondary, fontSize: 12, fontWeight: '600' }}>{i}</Text>
                  </TouchableOpacity>
                ))}
              </View>

              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 }}>
                <Text style={{ color: colors.textSecondary, fontSize: 12 }}>Not likely</Text>
                <Text style={{ color: colors.success, fontSize: 12 }}>Very likely</Text>
              </View>
